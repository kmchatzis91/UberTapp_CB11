...
namespace UberTappDeveloping.Controllers
{
	...
	public class AccountController : Controller
	{
		...
		{
			string failedLogInMesg = "Failed login attempt :( Invalid Email or Password!";
			...
			try
			{
				var user = context.Users.Single(u => u.Email == viewModel.Email);
				...
				switch (result)
				{
					...
					default:
						ModelState.AddModelError("", failedLogInMesg);
						return View(viewModel);
				}
			}
			catch (Exception)
		...
		public async Task<ActionResult> Register(RegisterViewModel viewModel)
		{
			if (ModelState.IsValid)
			{
				...
				{
					// Give a Role //
					if (viewModel.IsVenueOwner == true)
					{
						var roleStore = new RoleStore<IdentityRole>(new ApplicationDbContext());
						var roleManager = new RoleManager<IdentityRole>(roleStore);
						await roleManager.CreateAsync(new IdentityRole("Venue Owner"));
						await UserManager.AddToRoleAsync(user.Id, "Venue Owner");
					}
					else
					{
						var roleStore = new RoleStore<IdentityRole>(new ApplicationDbContext());
						var roleManager = new RoleManager<IdentityRole>(roleStore);
						await roleManager.CreateAsync(new IdentityRole("Beer Enthusiast"));
						await UserManager.AddToRoleAsync(user.Id, "Beer Enthusiast");
					}
